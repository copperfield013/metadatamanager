seajs.use(['dialog', 'ajax', '$CPF'], function(Dialog, Ajax, $CPF) {
	var $page = $("#expression-tree");
	 ...
	 $(function(){
		  //FILTERS
		    ...
		    debugger;
		   var expressionId =  $page.find(".tree_view_panel").attr("expressionId");
		    ...
		    if (expressionId) {
                ...
                initNode(data.statExpression, parent);//值初始化第一个节点
     ...
     * 加载本身节点
     */
    function initNode(statExpression, parent){
    	$CPF.showLoading();
    	$(parent).html(""); //清空子节点
...
//				"</ul>" + 
	  ...
	  //初始化
	    function initATTRVAR(statExpression, parent){
	    	$CPF.showLoading();
	    	var sourcecode = $(parent).closest(".tree_view_panel").attr("sourcecode");
	        var dragWrapLen = $(".dragEdit-wrap").length + 1 ;
	        $CPF.showLoading();
	        var nodeHtml = "<li class='attr-relative' data-id='"+statExpression.id+"'>" + 
...
//							"</ul>" + 
    ...
    function addFilters(el, source) {
        var $content = $(el).closest(".collapse-header").siblings(".collapse-content");
        var entityId = $(el).closest(".tree_view_panel").attr("parentId");
        ...
        if(entityTitle.length > 0) {
			...
			if(page.getPageObj() instanceof Dialog){
				...
				if(typeof afterSave == 'function'){
					afterSave.apply(page, [expressionId]);
				}
			}
